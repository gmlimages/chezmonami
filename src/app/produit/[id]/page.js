...
import { JsonLd, produitSchema, breadcrumbSchema } from '@/components/JsonLd';
...
export default function ProduitDetail() {
  ...
  const enregistrerCommande = async (methode) => {
    try {
      ...
      // Notification email (client + boutique)
  ...
  const envoyerCommandeEmail = async () => {
    if (!formulaireCommande.nom || !formulaireCommande.telephone || !formulaireCommande.email || !formulaireCommande.adresseLivraison) {
      toast.warning(t('produit_detail.toast_champs_obligatoires'));
      return;
    }
...
`;
    ...
    toast.success(t('produit_detail.toast_succes_email'));
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">{t('produit_detail.chargement')}</p>
        </div>
      </div>
    );
  }
  ...
  if (!produit) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-xl text-gray-600">{t('produit_detail.introuvable')}</p>
          <Link href="/boutique" className="btn-accent mt-4">
            {t('produit_detail.retour_boutique')}
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      {produit && <JsonLd data={produitSchema(produit)} />}
      {produit && (
        <JsonLd
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="mb-6 flex items-center gap-2 text-sm text-gray-600">
          <Link href="/" className="hover:text-accent">{t('produit_detail.breadcrumb_accueil')}</Link>
          <span>/</span>
          <Link href="/boutique" className="hover:text-accent">{t('produit_detail.breadcrumb_boutique')}</Link>
          <span>/</span>
            ...
            <div>
              <div className="flex items-start justify-between gap-4 mb-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <BoutonFavori type="produits" id={produit.id} variant="inline" showLabel />
                  <BoutonPartage titre={produit.nom} description={produit.description} variant="compact" />
                  {produit.structure?.id && (
                    <BoutonContacterEspace
                      structureId={produit.structure.id}
                      type="produit"
                      nom={produit.nom}
                      variant="outline"
                    />
              ...
              {produit.description && (
                <div className="mb-6">
                  <h3 className="font-bold text-gray-800 mb-2">{t('produit_detail.description')}</h3>
              ...
              {Object.entries(variationsParType).map(([type, variations]) => (
                <div key={type} className="mb-6">
                  <h3 className="font-bold text-gray-800 mb-3 capitalize">
                    {type === 'taille' ? t('produit_detail.taille') : type === 'couleur' ? t('produit_detail.couleur') : type}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {variations.map((variation, index) => (
                      ...
                      >
                        {variation.valeur}
                        {variation.stock === 0 && ` (${t('produit_detail.epuise')})`}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {/* Quantité */}
              <div className="mb-6">
                <h3 className="font-bold text-gray-800 mb-3">{t('produit_detail.quantite')}</h3>
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-sm text-gray-600">
                    {stockDisponible} {stockDisponible > 1 ? t('produit_detail.disponible_pluriel') : t('produit_detail.disponible_singulier')}
                  </span>
                </div>
              </div>
              ...
              {stockDisponible > 0 ? (
                <div className="space-y-3">
                  ...
                  >
                    ...
                    {t('produit_detail.ajouter_panier')}
                  </button>
                  ...
                  >
                    ...
                    {t('produit_detail.commander_whatsapp')}
                  </button>
                  ...
                  >
                    ...
                    {t('produit_detail.commander_email')}
                  </button>
                </div>
              ) : (
                <div className="text-center p-4 bg-red-50 rounded-lg">
                  <p className="text-red-600 font-semibold">{t('produit_detail.rupture')}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {produitsSimilaires.length > 0 && (
          <div className="mt-12">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">
              {t('produit_detail.similaires')} ({produitsSimilaires.length})
            </h2>
        ...
        {produit.structure && (
          <div className="mt-8">
            ...
            >
              {t('produit_detail.voir_tous_produits')} {produit.structure.nom}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b flex items-center justify-between sticky top-0 bg-white">
              <h2 className="text-2xl font-bold">{t('produit_detail.finaliser')}</h2>
            ...
            <div className="p-6">
              <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                <h3 className="font-bold mb-3">{t('produit_detail.votre_commande')}</h3>
                <div className="text-sm mb-2">
                  ...
                  {Object.entries(selectionsVariations).map(([type, variation]) => (
                    <p key={type} className="text-gray-600 capitalize">
                      {type === 'taille' ? t('produit_detail.taille').replace('📏 ', '') : type === 'couleur' ? t('produit_detail.couleur').replace('🎨 ', '') : type}: {variation.valeur}
                    </p>
                  ))}
                  <p className="text-gray-600">{t('produit_detail.quantite_label')}: {quantite}</p>
                </div>
                <div className="border-t mt-2 pt-2 flex justify-between font-bold text-accent">
                  <span>{t('produit_detail.total')}</span>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t('produit_detail.nom_complet')}</label>
                  <input
                    type="text"
                    placeholder={t('produit_detail.placeholder_nom')}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t('produit_detail.telephone')}</label>
                  <input
                    type="tel"
                    placeholder={t('produit_detail.placeholder_tel')}
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-accent focus:outline-none"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t('produit_detail.email_label')}</label>
                  <input
                    type="email"
                    placeholder={t('produit_detail.placeholder_email')}
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-accent focus:outline-none"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t('produit_detail.adresse_livraison')}</label>
                  <textarea
                    rows="3"
                    placeholder={t('produit_detail.placeholder_adresse')}
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-accent focus:outline-none"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t('produit_detail.message_optionnel')}</label>
                  <textarea
                    rows="3"
                    placeholder={t('produit_detail.placeholder_message')}
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-accent focus:outline-none"
                ...
                <button onClick={envoyerCommandeEmail} className="w-full btn-accent py-4 text-lg">
                  {t('produit_detail.envoyer_par_email')}
                </button>

                <p className="text-xs text-gray-500 text-center">
                  {t('produit_detail.info_email')}
                </p>
              </div>
            </div>
          </div>
